...
# Preplot the pedestrian space
# par(mfrow=c(1,1))
# space <- objects[[1]]
# nams <- unlist(lapply(strsplit(row.names(p_n),"_"),function(x){x[1]}))
# plot(p_n,pch=nams, xlim=space$x, ylim=space$y)
# rect(space$x[1],space$y[1],space$x[2],space$y[2])
# grid(nx=AAl,ny=AAw,col="gray",lty="dotted")
